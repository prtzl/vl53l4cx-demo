...
int8_t VL53LX_trace_config(
    char* filename, uint32_t modules, uint32_t level, uint32_t functions)
{
    int8_t status = 0;
    ...
    if (((filename != NULL) && (_tracefile == NULL)) && strcmp(filename, ""))
    {
        ...
        // TODO: Add time and header banner to the log file to indicate we've just opened a new
        // log session
...
void VL53LX_trace_print_module_function(
    uint32_t module, uint32_t level, uint32_t function, const char* format, ...)
{
        ...
        || ((function & _trace_functions) > 0))
    {
        ...
        vsnprintf(
            message,
            VL53LX_MAX_STRING_LENGTH - 1,
            format,
            arg_list); /*lint !e534  ignore return*/
        va_end(arg_list);
