...
extern "C"
{
    ...
    void VL53LX_init_ll_driver_state(VL53LX_DEV Dev, VL53LX_DeviceState ll_state);
    ...
    uint32_t VL53LX_calc_decoded_timeout_us(
        uint16_t timeout_encoded, uint32_t macro_period_us);
    ...
    void VL53LX_encode_unsigned_integer(
        uint32_t ip_value, uint8_t no_of_bytes, uint8_t* pbuffer);
    ...
    uint16_t VL53LX_hist_xtalk_shape_model_interp(
        uint16_t events_per_bin, uint32_t phase_delta);
    ...
    VL53LX_Error VL53LX_get_spad_rate_data(
        VL53LX_DEV Dev, VL53LX_spad_rate_data_t* pspad_rates);
