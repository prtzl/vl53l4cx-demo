...
#define trace_print(level, ...)                                                               \
    _LOG_TRACE_PRINT(                                                                         \
        VL53LX_TRACE_MODULE_CORE, level, VL53LX_TRACE_FUNCTION_NONE, ##__VA_ARGS__)
    ...
    else
    {
        ...
        case VL53LX_DEVICESTATE_SW_STANDBY:
            ...
            if ((pdev->dyn_cfg.system__grouped_parameter_hold
                ...
                > 0)
            {
                pstate->rd_device_state = VL53LX_DEVICESTATE_RANGING_WAIT_GPH_SYNC;
            }
            else
            {
...
VL53LX_Error VL53LX_check_ll_driver_rd_state(VL53LX_DEV Dev)
{
    ...
    device_range_status =
        psys_results->result__range_status & VL53LX_RANGE_STATUS__RANGE_STATUS_MASK;

    device_stream_count = psys_results->result__stream_count;

    histogram_mode = (pdev->sys_ctrl.system__mode_start & VL53LX_DEVICESCHEDULERMODE_HISTOGRAM)
    ...
    if (!((pdev->sys_ctrl.system__mode_start & VL53LX_DEVICEMEASUREMENTMODE_BACKTOBACK)
    ...
    else
    {
        ...
        case VL53LX_DEVICESTATE_RANGING_DSS_AUTO:
            ...
            if (pstate->cfg_zone_id > pdev->zone_cfg.active_zones)
            {
                ...
                {
                    pstate->cfg_device_state = VL53LX_DEVICESTATE_RANGING_DSS_MANUAL;
                }
            }
            break;
...
void VL53LX_init_xtalk_bin_data_struct(
    uint32_t bin_value, uint16_t VL53LX_p_021, VL53LX_xtalk_histogram_shape_t* pdata)
{
...
uint32_t VL53LX_i2c_decode_with_mask(
    uint16_t count, uint8_t* pbuffer, uint32_t bit_mask, uint32_t down_shift, uint32_t offset)
{
...
VL53LX_Error VL53LX_clear_interrupt(VL53LX_DEV Dev)
{
    ...
    status = VL53LX_WrByte(
        Dev, VL53LX_SYSTEM__INTERRUPT_CLEAR, pdev->sys_ctrl.system__interrupt_clear);
...
VL53LX_Error VL53LX_force_shadow_stream_count_to_zero(VL53LX_DEV Dev)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        status = VL53LX_WrByte(Dev, VL53LX_SHADOW_RESULT__STREAM_COUNT, 0x00);
    }
...
uint16_t VL53LX_calc_range_ignore_threshold(
    uint32_t central_rate, int16_t x_gradient, int16_t y_gradient, uint8_t rate_mult)
{
...
uint32_t VL53LX_calc_decoded_timeout_us(uint16_t timeout_encoded, uint32_t macro_period_us)
{
...
uint32_t VL53LX_decode_timeout(uint16_t encoded_timeout)
{
    ...
    timeout_macro_clks =
        ((uint32_t)(encoded_timeout & 0x00FF) << (uint32_t)((encoded_timeout & 0xFF00) >> 8))
        + 1;
    ...
    VL53LX_timing_config_t* ptiming)
{
    ...
    else
    {
        ...
        macro_period_us = VL53LX_calc_macro_period_us(
            fast_osc_frequency, ptiming->range_config__vcsel_period_a);
        ...
        timeout_encoded =
            VL53LX_calc_encoded_timeout(range_config_timeout_us, macro_period_us);
        ...
        macro_period_us = VL53LX_calc_macro_period_us(
            fast_osc_frequency, ptiming->range_config__vcsel_period_b);
...
uint32_t VL53LX_decode_unsigned_integer(uint8_t* pbuffer, uint8_t no_of_bytes)
{
...
void VL53LX_encode_unsigned_integer(uint32_t ip_value, uint8_t no_of_bytes, uint8_t* pbuffer)
{
...
VL53LX_Error VL53LX_hist_copy_and_scale_ambient_info(
    VL53LX_zone_hist_info_t* pidata, VL53LX_histogram_bin_data_t* podata)
{
    ...
    {
        if (pidata->number_of_ambient_bins > 0 && podata->number_of_ambient_bins == 0)
        {
    ...
    VL53LX_range_results_t* prange_curr)
{
    ...
    for (lc = 0; lc < prange_curr->active_results; lc++)
    {
        ...
        if (!((prange_curr->VL53LX_p_003[lc].range_status == VL53LX_DEVICEERROR_RANGECOMPLETE)
        ...
        for (p = 0; p < prange_prev->active_objects; p++)
        {
            ...
            {
                phase_delta = prange_curr->VL53LX_p_003[lc].VL53LX_p_011
                            - prange_prev->VL53LX_p_003[p].VL53LX_p_011;
            }
            else
            {
                phase_delta = prange_prev->VL53LX_p_003[p].VL53LX_p_011
            ...
            if (phase_delta < phase_tolerance)
            {
                ...
                if (status == VL53LX_ERROR_NONE
                    && range_status == VL53LX_DEVICEERROR_RANGECOMPLETE)
                    status = VL53LX_hist_merged_pulse_check(
                        min_max_tolerance, &(prange_curr->VL53LX_p_003[lc]), &range_status);
    ...
    VL53LX_DeviceError* prange_status)
{
    ...
    noise_sq_sum = (uint32_t)prange_prev->VL53LX_p_017 + (uint32_t)c_sig_noise_sq
                 + (uint32_t)p_amb_noise_sq + (uint32_t)c_amb_noise_sq;
    ...
    int16_t* prange_offset_mm)
{
    ...
    if ((max_mm_inner_effective_spads == 0) || (max_mm_outer_effective_spads == 0))
        goto FAIL;
...
VL53LX_Error VL53LX_hist_xtalk_extract_calc_event_sums(
    VL53LX_histogram_bin_data_t* phist_bins, VL53LX_hist_xtalk_extract_data_t* pxtalk_data)
{
    ...
    for (lb = pxtalk_data->VL53LX_p_012; lb <= pxtalk_data->VL53LX_p_013; lb++)
    {
        ...
        i = (lb + phist_bins->number_of_ambient_bins + phist_bins->VL53LX_p_021)
    ...
    VL53LX_hist_xtalk_extract_data_t* pxtalk_data)
{
    ...
    if (pxtalk_data->signal_events_sum < 0)
    {
        ...
        tmp64_0 = ((uint64_t)pxtalk_data->sample_count
    ...
    tmp64_0 = ((uint64_t)pxtalk_data->signal_events_sum * (uint64_t)pxtalk_data->sample_count
               * (uint64_t)pxtalk_data->event_scaler_avg * 256U)
           << 9U;
    ...
    VL53LX_xtalk_histogram_shape_t* pxtalk_shape)
{
    ...
    if (pxtalk_data->signal_events_sum > 0)
        total_events =
            (uint64_t)pxtalk_data->signal_events_sum * (uint64_t)pxtalk_data->event_scaler_avg;
    else
        total_events = 1;
    if (total_events == 0)
        total_events = 1;
    ...
    VL53LX_xtalk_histogram_shape_t* pxtalk_shape)
{
    ...
    pxtalk_shape->phasecal_result__reference_phase = pulse_centre + (3 * 2048);
    ...
    for (lb = 0; lb < VL53LX_XTALK_HISTO_BINS; lb++)
    {
        ...
        {
            VL53LX_p_018 =
                VL53LX_hist_xtalk_shape_model_interp(events_per_bin, phase_stop - phase_start);
        }
        ...
        else if (lb == bin_stop)
        {
            ...
            phase_bin = lb * 2048;
            VL53LX_p_018 =
                VL53LX_hist_xtalk_shape_model_interp(events_per_bin, (phase_stop - phase_bin));
        }
...
uint16_t VL53LX_hist_xtalk_shape_model_interp(uint16_t events_per_bin, uint32_t phase_delta)
{
    ...
    for (y = zone_y_ll; y <= zone_y_ur; y++)
    {
        for (x = zone_x_ll; x <= zone_x_ur; x++)
        {
            ...
            VL53LX_spad_number_to_byte_bit_index(
                spad_number, &byte_index, &bit_index, &bit_mask);
    ...
    uint16_t* pmm_outer_effective_spads)
{
    ...
    VL53LX_decode_zone_limits(
        encoded_mm_roi_centre, encoded_mm_roi_size, &mm_x_ll, &mm_y_ll, &mm_x_ur, &mm_y_ur);
...
VL53LX_Error VL53LX_sum_histogram_data(
    VL53LX_histogram_bin_data_t* phist_input, VL53LX_histogram_bin_data_t* phist_output)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        if (phist_output->VL53LX_p_021 >= phist_input->VL53LX_p_021)
    ...
    if (status == VL53LX_ERROR_NONE)
        phist_output->VL53LX_p_028 += phist_input->VL53LX_p_028;
    ...
    VL53LX_histogram_bin_data_t* phist_avg)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        for (i = 0; i < phist_sum->VL53LX_p_021; i++)
        {
            ...
            if (no_of_samples > 0)
                phist_avg->bin_data[i] = phist_sum->bin_data[i] / (int32_t)no_of_samples;
            else
    ...
    {
        if (no_of_samples > 0)
            phist_avg->VL53LX_p_028 = phist_sum->VL53LX_p_028 / (int32_t)no_of_samples;
        else
...
VL53LX_Error VL53LX_dynamic_zone_update(VL53LX_DEV Dev, VL53LX_range_results_t* presults)
{
    ...
    uint16_t eff_spad_cnt = pZ->VL53LX_p_003[zone_id].dss_requested_effective_spad_count;
#endif
    ...
    trace_print(VL53LX_TRACE_LEVEL_DEBUG, "    DYNZONEUPDATE: peak signal count rate mcps:");
    ...
    else
    {
        ...
        temp = target_rate << 14;
        trace_print(VL53LX_TRACE_LEVEL_DEBUG, "    DYNZONEUPDATE: 1: temp: %u\n", temp);

        temp = temp / max_total_rate_per_spads;

        trace_print(VL53LX_TRACE_LEVEL_DEBUG, "    DYNZONEUPDATE: 2: temp: %u\n", temp);

        if (temp > 0xFFFF)
            temp = 0xFFFF;

        trace_print(VL53LX_TRACE_LEVEL_DEBUG, "    DYNZONEUPDATE: 3: temp: %u\n", temp);
    }
...
VL53LX_Error VL53LX_multizone_hist_bins_update(VL53LX_DEV Dev)
{
    ...
    VL53LX_histogram_config_t* pmulti_hist = &(pzone_cfg->multizone_hist_cfg);
        ...
        == VL53LX_ZONECONFIG_BINCONFIG__LOWAMB)
    {
        ...
        pzone_cfg->bin_config[pdev->ll_state.cfg_zone_id]
        == VL53LX_ZONECONFIG_BINCONFIG__MIDAMB)
    {
        trace_print(
            VL53LX_TRACE_LEVEL_DEBUG, "    HISTBINCONFIGUPDATE: Setting MIDAMB timing\n");
        if (!next_range_is_odd_timing)
        {
            trace_print(
                VL53LX_TRACE_LEVEL_DEBUG,
    ...
    VL53LX_histogram_config_t* pmulti_hist)
{
    ...
    if (pzone_cfg->bin_config[0] == VL53LX_ZONECONFIG_BINCONFIG__LOWAMB)
    {
        phist_cfg->histogram_config__low_amb_even_bin_0_1 =
            pmulti_hist->histogram_config__low_amb_even_bin_0_1;
        phist_cfg->histogram_config__low_amb_even_bin_2_3 =
    ...
    else if (pzone_cfg->bin_config[0] == VL53LX_ZONECONFIG_BINCONFIG__MIDAMB)
    {
        phist_cfg->histogram_config__low_amb_even_bin_0_1 =
            pmulti_hist->histogram_config__mid_amb_even_bin_0_1;
        phist_cfg->histogram_config__low_amb_even_bin_2_3 =
    ...
    else if (pzone_cfg->bin_config[0] == VL53LX_ZONECONFIG_BINCONFIG__HIGHAMB)
    {
        phist_cfg->histogram_config__low_amb_even_bin_0_1 =
            pmulti_hist->histogram_config__high_amb_even_bin_0_1;
        phist_cfg->histogram_config__low_amb_even_bin_2_3 =
...
uint8_t VL53LX_encode_GPIO_interrupt_config(VL53LX_GPIO_interrupt_config_t* pintconf)
{
...
VL53LX_Error VL53LX_set_GPIO_distance_threshold(
    VL53LX_DEV Dev, uint16_t threshold_high, uint16_t threshold_low)
{
    VL53LX_Error status = VL53LX_ERROR_NONE;
...
VL53LX_Error VL53LX_set_GPIO_thresholds_from_struct(
    VL53LX_DEV Dev, VL53LX_GPIO_interrupt_config_t* pintconf)
{
    VL53LX_Error status = VL53LX_ERROR_NONE;
    ...
    status = VL53LX_set_GPIO_distance_threshold(
        Dev, pintconf->threshold_distance_high, pintconf->threshold_distance_low);

    if (status == VL53LX_ERROR_NONE)
    {
        status = VL53LX_set_GPIO_rate_threshold(
            Dev, pintconf->threshold_rate_high, pintconf->threshold_rate_low);
    }
    ...
    uint16_t fast_osc_frequency)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WrWord(
            Dev, VL53LX_REF_SPAD_CHAR__TOTAL_RATE_TARGET_MCPS, total_rate_target_mcps);

    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WrWord(
            Dev, VL53LX_RANGE_CONFIG__SIGMA_THRESH, max_count_rate_rtn_limit_mcps);
...
VL53LX_Error VL53LX_set_ssc_config(
    VL53LX_DEV Dev, VL53LX_ssc_config_t* pssc_cfg, uint16_t fast_osc_frequency)
{
    ...
    macro_period_us = VL53LX_calc_macro_period_us(fast_osc_frequency, pssc_cfg->VL53LX_p_005);

    timeout_encoded = VL53LX_calc_encoded_timeout(pssc_cfg->timeout_us, macro_period_us);

    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WrByte(Dev, VL53LX_CAL_CONFIG__VCSEL_START, pssc_cfg->vcsel_start);

    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WrByte(Dev, VL53LX_GLOBAL_CONFIG__VCSEL_WIDTH, pssc_cfg->vcsel_width);
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WriteMulti(Dev, VL53LX_RANGE_CONFIG__TIMEOUT_MACROP_B_HI, buffer, 5);
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WrByte(Dev, VL53LX_NVM_BIST__CTRL, pssc_cfg->array_select);
...
VL53LX_Error VL53LX_get_spad_rate_data(VL53LX_DEV Dev, VL53LX_spad_rate_data_t* pspad_rates)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_ReadMulti(Dev, VL53LX_PRIVATE__PATCH_BASE_ADDR_RSLV, pdata, 512);

    pdata = &VL53LX_p_003[0];
    for (i = 0; i < VL53LX_NO_OF_SPAD_ENABLES; i++)
    {
        pspad_rates->rate_data[i] = (uint16_t)VL53LX_decode_unsigned_integer(pdata, 2);
        pdata += 2;
    }
    ...
    uint8_t soft_update)
{
    ...
    if (add_smudge == 1)
    {
        pout->algo__crosstalk_compensation_plane_offset_kcps =
            (uint32_t)xtalk_offset_out + (uint32_t)pconfig->smudge_margin;
    }
    else
    {
    ...
    if (((pconfig->user_scaler_set == 0) || (pconfig->scaler_calc_method == 1))
    ...
    else
    {
        ...
        x_gradient_scaler =
            (int16_t)do_division_s((((int32_t)orig_x_gradient) << 6), orig_xtalk_offset);
        pconfig->x_gradient_scaler = x_gradient_scaler;
        y_gradient_scaler =
            (int16_t)do_division_s((((int32_t)orig_y_gradient) << 6), orig_xtalk_offset);
    ...
    if (pconfig->scaler_calc_method == 0)
    {
        ...
        itemp32 =
            (int32_t)(pout->algo__crosstalk_compensation_plane_offset_kcps * x_gradient_scaler);
        itemp32 = itemp32 >> 6;
        if (itemp32 > 0xFFFF)
            itemp32 = 0xFFFF;
        ...
        itemp32 =
            (int32_t)(pout->algo__crosstalk_compensation_plane_offset_kcps * y_gradient_scaler);
        itemp32 = itemp32 >> 6;
        if (itemp32 > 0xFFFF)
            itemp32 = 0xFFFF;
    ...
    else if (pconfig->scaler_calc_method == 1)
    {
        ...
        itemp32 =
            (int32_t)(orig_xtalk_offset - pout->algo__crosstalk_compensation_plane_offset_kcps);
    ...
    if ((pconfig->smudge_corr_apply_enabled == 1) && (soft_update != 1))
    {
        ...
        {
            cXtalk = pC->algo__xtalk_cpo_HistoMerge_kcps[histo_merge_nb - 1];
...
VL53LX_Error VL53LX_dynamic_xtalk_correction_corrector(VL53LX_DEV Dev)
{
    ...
    run_smudge_detection = (pconfig->smudge_corr_enabled == 1) && ambient_check
    ...
    if (run_smudge_detection)
    {
        ...
        if (continue_processing == CONT_CONTINUE)
        {
            ...
            VL53LX_dynamic_xtalk_correction_calc_new_xtalk(
                Dev, xtalk_offset_out, pconfig, pout, 1, 0);
        ...
        else
        {
            ...
            VL53LX_dynamic_xtalk_correction_calc_new_xtalk(
                Dev, xtalk_offset_out, pconfig, pout, 1, 1);
        }
    ...
    if (run_nodetect == 1)
    {
        ...
        if (continue_processing == CONT_CONTINUE)
        {
            ...
            VL53LX_dynamic_xtalk_correction_calc_new_xtalk(
                Dev, xtalk_offset_out, pconfig, pout, 0, 0);
        ...
        else
        {
            ...
            VL53LX_dynamic_xtalk_correction_calc_new_xtalk(
                Dev, xtalk_offset_out, pconfig, pout, 0, 1);
        }
    ...
    VL53LX_low_power_auto_data_t* plpadata)
{
    ...
    pdynamic->system__sequence_config = VL53LX_SEQUENCE_VHV_EN | VL53LX_SEQUENCE_PHASECAL_EN
                                      | VL53LX_SEQUENCE_DSS1_EN |

                                        VL53LX_SEQUENCE_RANGE_EN;

    pgeneral->dss_config__manual_effective_spads_select = 200 << 8;
    pgeneral->dss_config__roi_mode_control = VL53LX_DEVICEDSSMODE__REQUESTED_EFFFECTIVE_SPADS;
