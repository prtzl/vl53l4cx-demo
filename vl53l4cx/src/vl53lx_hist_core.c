    ...
    uint8_t histo_merge_nb)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        ...
        pdata->peak_signal_count_rate_mcps =
            VL53LX_rate_maths((int32_t)pdata->VL53LX_p_010, pdata->peak_duration_us);

        pdata->avg_signal_count_rate_mcps =
            VL53LX_rate_maths((int32_t)pdata->VL53LX_p_010, pdata->woi_duration_us);

        pdata->ambient_count_rate_mcps =
            VL53LX_rate_maths((int32_t)pdata->VL53LX_p_016, pdata->woi_duration_us);

        count_rate_total = (uint32_t)pdata->peak_signal_count_rate_mcps
        ...
        pdata->total_rate_per_spad_mcps =
            VL53LX_rate_per_spad_maths(0x06, count_rate_total, pdata->VL53LX_p_004, 0xFFFF);

        pdata->VL53LX_p_009 = VL53LX_events_per_spad_maths(
            pdata->VL53LX_p_010, pdata->VL53LX_p_004, pdata->peak_duration_us);
...
void VL53LX_f_019(uint16_t gain_factor, int16_t range_offset_mm, VL53LX_range_data_t* pdata)
{
    ...
    pdata->min_range_mm = (int16_t)VL53LX_range_maths(
    ...
    pdata->median_range_mm = (int16_t)VL53LX_range_maths(
        pdata->fast_osc_frequency,
        pdata->VL53LX_p_011,
        pdata->zero_distance_phase,
        0,
    ...
    pdata->max_range_mm = (int16_t)VL53LX_range_maths(
        pdata->fast_osc_frequency,
        pdata->VL53LX_p_027,
        pdata->zero_distance_phase,
        0,
...
void VL53LX_f_029(VL53LX_histogram_bin_data_t* pdata, int32_t ambient_estimate_counts_per_bin)
{

    uint8_t i = 0;

    for (i = 0; i < pdata->VL53LX_p_021; i++)
        pdata->bin_data[i] = pdata->bin_data[i] - ambient_estimate_counts_per_bin;
}
    ...
    VL53LX_histogram_bin_data_t* pxtalk_realigned)
{
    ...
    for (i = 0; i < min_bins; i++)
    {
        ...
        if (bin_offset >= 0)
            ...
            bin_access = ((int8_t)pbins->VL53LX_p_021 + ((int8_t)i + (int8_t)bin_offset))
...
VL53LX_Error VL53LX_f_031(
    VL53LX_histogram_bin_data_t* pidata, VL53LX_histogram_bin_data_t* podata)
{
    ...
    for (lc = 0; lc < VL53LX_MAX_BIN_SEQUENCE_LENGTH; lc++)
    {
        ...
        for (i = 0; i < 4; i++)
            podata->bin_data[VL53LX_p_032 + i] += pidata->bin_data[lc * 4 + i];
    }
    ...
    for (lc = 0; lc < VL53LX_MAX_BIN_SEQUENCE_LENGTH; lc++)
    {
        ...
        if (bin_cfg <= VL53LX_MAX_BIN_SEQUENCE_CODE)
            podata->bin_rep[lc] = bin_repeat_count[bin_cfg];
        else
            podata->bin_rep[lc] = 0;
    }
