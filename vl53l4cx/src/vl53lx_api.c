...
#define LOG_FUNCTION_END_FMT(status, fmt, ...)                                                \
    _LOG_FUNCTION_END_FMT(VL53LX_TRACE_MODULE_API, status, fmt, ##__VA_ARGS__)

#ifdef VL53LX_LOG_ENABLE
#define trace_print(level, ...)                                                               \
    trace_print_module_function(                                                              \
        VL53LX_TRACE_MODULE_API, level, VL53LX_TRACE_FUNCTION_NONE, ##__VA_ARGS__)
#endif
...
static VL53LX_Error ComputeDevicePresetMode(
    VL53LX_DistanceModes DistanceMode, VL53LX_DevicePresetModes* pDevicePresetMode);

static VL53LX_Error SetPresetModeL3CX(
    VL53LX_DEV Dev, VL53LX_DistanceModes DistanceMode, uint32_t inter_measurement_period_ms);
...
VL53LX_Error VL53LX_GetProductRevision(
    VL53LX_DEV Dev, uint8_t* pProductRevisionMajor, uint8_t* pProductRevisionMinor)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
    uint8_t revision_id;
...
VL53LX_Error VL53LX_GetUID(VL53LX_DEV Dev, uint64_t* pUid)
{
    ...
    Status = VL53LX_read_nvm_raw_data(Dev, (uint8_t)(0x1F8 >> 2), (uint8_t)(8 >> 2), fmtdata);
...
VL53LX_Error VL53LX_SetDeviceAddress(VL53LX_DEV Dev, uint8_t DeviceAddress)
{
    ...
    Status = VL53LX_WrByte(Dev, VL53LX_I2C_SLAVE__DEVICE_ADDRESS, DeviceAddress / 2);
    ...
    VL53LX_DEV Dev, uint32_t* pMeasurementTimingBudgetMicroSeconds)
{
    ...
    *pMeasurementTimingBudgetMicroSeconds = 0;
    ...
    if (Status == VL53LX_ERROR_NONE)
        Status =
            VL53LX_get_timeouts_us(Dev, &PhaseCalTimeoutUs, &MmTimeoutUs, &RangeTimeoutUs);

    if (Status == VL53LX_ERROR_NONE)
        *pMeasurementTimingBudgetMicroSeconds = (6 * RangeTimeoutUs) + 1700;
...
VL53LX_Error VL53LX_StartMeasurement(VL53LX_DEV Dev)
{
    ...
    if (Status == VL53LX_ERROR_NONE)
        Status = VL53LX_init_and_start_range(
            Dev, DeviceMeasurementMode, VL53LX_DEVICECONFIGLEVEL_FULL);
...
static VL53LX_Error SetTargetData(
    VL53LX_DEV Dev,
    ...
    VL53LX_TargetRangeData_t* pRangeData)
{
    ...
    if (ExtendedRangeEnabled
        && (pRangeData->RangeStatus == VL53LX_RANGESTATUS_WRAP_TARGET_FAIL
            || pRangeData->RangeStatus == VL53LX_RANGESTATUS_OUTOFBOUNDS_FAIL)
        && (pdev->PreviousRangeStatus[iteration] == VL53LX_RANGESTATUS_WRAP_TARGET_FAIL
            || pdev->PreviousRangeStatus[iteration] == VL53LX_RANGESTATUS_OUTOFBOUNDS_FAIL
            || (pdev->PreviousRangeStatus[iteration] == VL53LX_RANGESTATUS_RANGE_VALID
                && pdev->PreviousExtendedRange[iteration] == 1)))
    {
        if (((pdev->PreviousStreamCount) == (pdev->hist_data.result__stream_count - 1))
            || ((pdev->PreviousStreamCount) == (pdev->hist_data.result__stream_count + 127)))
        {
            ...
            case VL53LX_DEVICEPRESETMODE_HISTOGRAM_MEDIUM_RANGE:
                if (RangeDiff > tp->tp_uwr_med_z_1_min && RangeDiff < tp->tp_uwr_med_z_1_max
                    && sequency == 1)
                {
                    AddOffset = tp->tp_uwr_med_corr_z_1_rangeb;
                }
                else if (
                    RangeDiff < -tp->tp_uwr_med_z_1_min && RangeDiff > -tp->tp_uwr_med_z_1_max
                    && sequency == 0)
                {
                    AddOffset = tp->tp_uwr_med_corr_z_1_rangea;
                }
                else if (
                    RangeDiff > tp->tp_uwr_med_z_2_min && RangeDiff < tp->tp_uwr_med_z_2_max
                    && sequency == 0)
                {
                    ...
                    && sequency == 1)
                {
                    ...
                    RangeDiff > tp->tp_uwr_med_z_3_min && RangeDiff < tp->tp_uwr_med_z_3_max
                    && sequency == 1)
                {
                    ...
                    && sequency == 0)
                {
                    ...
                    RangeDiff > tp->tp_uwr_med_z_4_min && RangeDiff < tp->tp_uwr_med_z_4_max
                    && sequency == 0)
                {
                    ...
                    && sequency == 1)
                {
                    ...
                    RangeDiff < tp->tp_uwr_med_z_5_max && RangeDiff > tp->tp_uwr_med_z_5_min)
                {
            ...
            case VL53LX_DEVICEPRESETMODE_HISTOGRAM_LONG_RANGE:
                if (RangeDiff > tp->tp_uwr_lng_z_1_min && RangeDiff < tp->tp_uwr_lng_z_1_max
                    && sequency == 0)
                {
                    AddOffset = tp->tp_uwr_lng_corr_z_1_rangea;
                }
                else if (
                    RangeDiff < -tp->tp_uwr_lng_z_1_min && RangeDiff > -tp->tp_uwr_lng_z_1_max
                    && sequency == 1)
                {
                    AddOffset = tp->tp_uwr_lng_corr_z_1_rangeb;
                }
                else if (
                    RangeDiff > tp->tp_uwr_lng_z_2_min && RangeDiff < tp->tp_uwr_lng_z_2_max
                    && sequency == 1)
                {
                    ...
                    && sequency == 0)
                {
                    ...
                    RangeDiff < tp->tp_uwr_lng_z_3_max && RangeDiff > tp->tp_uwr_lng_z_3_min)
                {
...
static VL53LX_Error SetMeasurementData(
    VL53LX_DEV Dev,
...
VL53LX_Error VL53LX_GetMultiRangingData(
    VL53LX_DEV Dev, VL53LX_MultiRangingData_t* pMultiRangingData)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
    VL53LX_LLDriverData_t* pdev = VL53LXDevStructGetLLDriverHandle(Dev);
...
VL53LX_Error VL53LX_SetTuningParameter(
    VL53LX_DEV Dev, uint16_t TuningParameterId, int32_t TuningParameterValue)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
...
VL53LX_Error VL53LX_GetTuningParameter(
    VL53LX_DEV Dev, uint16_t TuningParameterId, int32_t* pTuningParameterValue)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
...
VL53LX_Error VL53LX_PerformRefSpadManagement(VL53LX_DEV Dev)
{
    ...
    if (Status == VL53LX_WARNING_REF_SPAD_CHAR_RATE_TOO_HIGH)
    {
        ...
        Status =
            VL53LX_read_nvm_raw_data(Dev, (uint8_t)(0xA0 >> 2), (uint8_t)(24 >> 2), dcrbuffer);

        if (Status == VL53LX_ERROR_NONE)
            Status = VL53LX_WriteMulti(
                Dev, VL53LX_REF_SPAD_MAN__NUM_REQUESTED_REF_SPADS, numloc, 2);
        ...
        if (Status == VL53LX_ERROR_NONE)
            Status =
                VL53LX_WriteMulti(Dev, VL53LX_GLOBAL_CONFIG__SPAD_ENABLES_REF_0, commbuf, 6);
...
VL53LX_Error VL53LX_GetXTalkCompensationEnable(
    VL53LX_DEV Dev, uint8_t* pXTalkCompensationEnable)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
...
VL53LX_Error VL53LX_PerformXTalkCalibration(VL53LX_DEV Dev)
{
    ...
    CalDistanceMm = (int16_t)BDTable[VL53LX_TUNING_XTALK_FULL_ROI_TARGET_DISTANCE_MM];
    ...
    if ((UStatus == VL53LX_ERROR_XTALK_EXTRACTION_SIGMA_LIMIT_FAIL) || (sum > (1024 + Margin))
        || (sum < (1024 - Margin)) || (binok < 3))
    {
...
VL53LX_Error VL53LX_SetOffsetCorrectionMode(
    VL53LX_DEV Dev, VL53LX_OffsetCorrectionModes OffsetCorrectionMode)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
...
VL53LX_Error VL53LX_SetCalibrationData(
    VL53LX_DEV Dev, VL53LX_CalibrationData_t* pCalibrationData)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
    ...
    cal_data.struct_version =
        pCalibrationData->struct_version - VL53LX_ADDITIONAL_CALIBRATION_DATA_STRUCT_VERSION;
    ...
    Status =
        VL53LX_set_tuning_parm(Dev, VL53LX_TUNINGPARM_DYNXTALK_NODETECT_XTALK_OFFSET_KCPS, x);
    ...
    VL53LX_DEV Dev, VL53LX_CalibrationData_t* pCalibrationData)
{
    ...
    pCalibrationData->struct_version =
        cal_data.struct_version + VL53LX_ADDITIONAL_CALIBRATION_DATA_STRUCT_VERSION;
    ...
    VL53LX_DEV Dev, int32_t CalDistanceMilliMeter)
{
    ...
    VL53LX_DistanceModes DistMode[3] = {
        VL53LX_DISTANCEMODE_SHORT, VL53LX_DISTANCEMODE_MEDIUM, VL53LX_DISTANCEMODE_LONG};
    ...
    while ((Repeat < 3) && (Status == VL53LX_ERROR_NONE))
    {
        ...
        if ((Status == VL53LX_ERROR_NONE) && (offset_meas_range_A > 0))
        {
            ...
            meanDistance_mm =
                (int16_t)((sum_ranging_range_A + IncRounding) / offset_meas_range_A);
        ...
        if ((Status == VL53LX_ERROR_NONE) && (offset_meas_range_B > 0))
        {
            ...
            meanDistance_mm =
                (int16_t)((sum_ranging_range_B + IncRounding) / offset_meas_range_B);
...
VL53LX_Error VL53LX_GetOpticalCenter(
    VL53LX_DEV Dev, FixPoint1616_t* pOpticalCenterX, FixPoint1616_t* pOpticalCenterY)
{
    VL53LX_Error Status = VL53LX_ERROR_NONE;
    ...
    *pOpticalCenterY = 0;
    ...
    {
        *pOpticalCenterX =
            VL53LX_FIXPOINT44TOFIXPOINT1616(CalibrationData.optical_centre.x_centre);
        *pOpticalCenterY =
            VL53LX_FIXPOINT44TOFIXPOINT1616(CalibrationData.optical_centre.y_centre);
    }
