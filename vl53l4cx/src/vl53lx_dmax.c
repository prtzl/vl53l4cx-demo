...
#define LOG_FUNCTION_END_FMT(status, fmt, ...)                                                \
    _LOG_FUNCTION_END_FMT(VL53LX_TRACE_MODULE_PROTECTED, status, fmt, ##__VA_ARGS__)

#define trace_print(level, ...)                                                               \
    _LOG_TRACE_PRINT(                                                                         \
        VL53LX_TRACE_MODULE_PROTECTED, level, VL53LX_TRACE_FUNCTION_NONE, ##__VA_ARGS__)
    ...
    int16_t* pambient_dmax_mm)
{
    ...
    *pambient_dmax_mm = 0;
    ...
    if ((pbins->VL53LX_p_015 != 0) && (pbins->total_periods_elapsed != 0))
    {
        ...
        pdata->VL53LX_p_037 = VL53LX_duration_maths(
            pll_period_us, 1 << 4, VL53LX_RANGING_WINDOW_VCSEL_PERIODS, periods_elapsed);

        pdata->VL53LX_p_034 = VL53LX_rate_maths(pbins->VL53LX_p_028, pdata->VL53LX_p_037);

        pdata->VL53LX_p_033 = VL53LX_events_per_spad_maths(
            pbins->VL53LX_p_028,
    ...
    if ((pcal->ref__actual_effective_spads != 0) && (pbins->VL53LX_p_015 != 0)
        && (pcal->ref_reflectance_pc != 0) && (pbins->total_periods_elapsed != 0))
    {
        ...
        pdata->VL53LX_p_022 = (int16_t)VL53LX_f_002(
            tmp32,
            pdata->VL53LX_p_035,
        ...
        pdata->VL53LX_p_036 = (int16_t)VL53LX_f_002(
