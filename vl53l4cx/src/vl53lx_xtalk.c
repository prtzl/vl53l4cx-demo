...
#define LOG_FUNCTION_END_FMT(status, fmt, ...)                                                \
    _LOG_FUNCTION_END_FMT(VL53LX_TRACE_MODULE_HISTOGRAM, status, fmt, ##__VA_ARGS__)

#define trace_print(level, ...)                                                               \
    _LOG_TRACE_PRINT(                                                                         \
        VL53LX_TRACE_MODULE_HISTOGRAM, level, VL53LX_TRACE_FUNCTION_NONE, ##__VA_ARGS__)
    ...
    VL53LX_xtalk_calibration_results_t* pxtalk_cal)
{
    ...
    memcpy(
        pavg_bins,
        &(pxtalk_results->central_histogram_avg),
        sizeof(VL53LX_histogram_bin_data_t));

    if (status == VL53LX_ERROR_NONE)
        VL53LX_init_histogram_bin_data_struct(0, 0, &(pdebug->VL53LX_p_056));

    if (status == VL53LX_ERROR_NONE)
        VL53LX_init_histogram_bin_data_struct(0, 0, &(pdebug->VL53LX_p_057));
    ...
    if (pxtalk_data->no_of_samples > 0)
    {
        ...
        if (status == VL53LX_ERROR_NONE)
        {
            memcpy(&(pdebug->VL53LX_p_056), pavg_bins, sizeof(VL53LX_histogram_bin_data_t));
        }
    ...
    VL53LX_xtalk_histogram_shape_t* pxtalk_shape)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        VL53LX_f_043(6, pavg_bins->VL53LX_p_028, &ambient_thresh);

    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_f_044(
            pavg_bins, ambient_thresh, histogram__window_start, histogram__window_end);
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (status == VL53LX_ERROR_NONE)
        {
            ...
            else
            {
                ...
                pdebug->VL53LX_p_061[i] = (uint32_t)xtalk_per_spad[i];
            }
        }
    }
    ...
    VL53LX_xtalk_histogram_shape_t* pxtalk_histo)
{
    ...
    pxtalk_histo->VL53LX_p_021 = VL53LX_XTALK_HISTO_BINS;
    ...
    memcpy(&(pdebug->VL53LX_p_057), phist_data, sizeof(VL53LX_histogram_bin_data_t));
    ...
    for (i = 0; i < pxtalk_histo->VL53LX_p_021; i++)
    {
        ...
        {
            bin_data[i] = ((uint64_t)phist_data->bin_data[idx] << 10);
            tmpi32 = pxtalk_data->signal_total_events_avg / 2;
            bin_data[i] = bin_data[i] + (uint64_t)tmpi32;
            bin_data[i] =
                do_division_u(bin_data[i], (uint64_t)pxtalk_data->signal_total_events_avg);
        }
        else
        {
            bin_data[i] = 0;
    ...
    VL53LX_histogram_bin_data_t* pxtalk_count_data)
{
    ...
    memcpy(pop_hist_data, pip_hist_data, sizeof(VL53LX_histogram_bin_data_t));
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_f_033(
            pip_hist_data, &(pxtalk_shape->xtalk_shape), xtalk_rate_kcps, pxtalk_count_data);

    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_f_047(
            pop_hist_data, pxtalk_count_data, pip_hist_data->number_of_ambient_bins);
    ...
    uint32_t* xtalk_rate_kcps)
{
    ...
    trace_print(VL53LX_TRACE_LEVEL_DEBUG, "    %-48s : %10d\n", "Row", row);
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        xtalk_rate_ll = ((int64_t)bound_l_x * ((int64_t)xgradient))
                      + ((int64_t)bound_d_y * ((int64_t)ygradient));
        xtalk_rate_ll = do_division_s((xtalk_rate_ll + 1), 2);
        xtalk_rate_ll += ((int64_t)mean_offset * 4);

        xtalk_rate_ur = ((int64_t)bound_r_x * ((int64_t)xgradient))
                      + ((int64_t)bound_u_y * ((int64_t)ygradient));
        xtalk_rate_ur = do_division_s((xtalk_rate_ur + 1), 2);
        xtalk_rate_ur += ((int64_t)mean_offset * 4);
    }

    trace_print(
        VL53LX_TRACE_LEVEL_DEBUG, "    %-48s : %10d\n", "xtalk_rate_ll", xtalk_rate_ll);
    ...
    VL53LX_histogram_bin_data_t* pxtalkcount_data)
{
    ...
    trace_print(
        ...
        "    %-48s : %10d\n",
        "pk_duration_internal",
        phist_data->peak_duration_us);

    xtalk_events_per_spad = do_division_u(
        (((uint64_t)xtalk_rate_kcps * (uint64_t)phist_data->peak_duration_us) + 500), 1000);

    trace_print(
        VL53LX_TRACE_LEVEL_DEBUG,
        "    %-48s : %10d\n",
        "xtalk_events_per_spad",
        xtalk_events_per_spad);

    xtalk_total_events =
        xtalk_events_per_spad * (uint64_t)phist_data->result__dss_actual_effective_spads;
    ...
    uint8_t xtalk_bin_offset)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        for (i = xtalk_bin_offset; i < pxtalk_data->VL53LX_p_021; i++)
        {
    ...
    uint8_t VL53LX_p_024)
{
    ...
    first_bin_int = (first_bin_int) % pxtalk_data->VL53LX_p_021;

    last_bin_int = (last_bin_int) % pxtalk_data->VL53LX_p_021;
    ...
    if (first_bin_inc > last_bin_int)
    {
        ...
        first_bin_int = (first_bin_int + realign_bin) % pxtalk_data->VL53LX_p_021;
        last_bin_int = (last_bin_int + realign_bin) % pxtalk_data->VL53LX_p_021;

        pxtalk_data->zero_distance_phase =
            pxtalk_data->zero_distance_phase + ((uint16_t)realign_bin * 2048);
    }
    ...
    if (realign_bin > 0)
    {
        ...
        {
            realign_index =
                (pxtalk_data->VL53LX_p_021 - realign_bin + i) % pxtalk_data->VL53LX_p_021;
...
VL53LX_Error VL53LX_f_043(uint8_t sigma_mult, int32_t VL53LX_p_028, uint32_t* ambient_noise)
{
