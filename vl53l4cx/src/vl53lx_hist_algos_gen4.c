    ...
    uint8_t histo_merge_nb)
{
    ...
    VL53LX_hist_estimate_ambient_from_thresholded_bins(
        (int32_t)ppost_cfg->ambient_thresh_sigma0, &(palgo3->VL53LX_p_006));
    ...
    for (p = 0; p < palgo3->VL53LX_p_046; p++)
    {
        ...
        if (status == VL53LX_ERROR_NONE)
            status = VL53LX_f_011(
                p, &(palgo3->VL53LX_p_006), palgo3, pB->VL53LX_p_028, &(palgo3->VL53LX_p_048));

        if (status == VL53LX_ERROR_NONE)
        {
            status =
                VL53LX_f_011(p, &(palgo3->VL53LX_p_006), palgo3, 0, &(palgo3->VL53LX_p_049));
        }

        if (status == VL53LX_ERROR_NONE)
        {
            status =
                VL53LX_f_011(p, &(palgo3->VL53LX_p_047), palgo3, 0, &(palgo3->VL53LX_p_050));
        }
    ...
    for (lb = pdata->VL53LX_p_012; lb < pdata->VL53LX_p_013; lb++)
    {
        ...
        if (i < palgo3->VL53LX_p_021 && j < palgo3->VL53LX_p_021)
        {
            ...
            if (pfiltered->VL53LX_p_053[i] == 0 && pfiltered->VL53LX_p_054[i] == 0)
                ...
                pfiltered->VL53LX_p_053[i] < 0 && pfiltered->VL53LX_p_054[i] >= 0
                && pfiltered->VL53LX_p_053[j] >= 0 && pfiltered->VL53LX_p_054[j] < 0)
                pfiltered->VL53LX_p_040[i] = 1;
    ...
    uint32_t* pmean_phase)
{
    ...
    VL53LX_p_055 =
        4096 * ((int64_t)VL53LX_p_001 - (int64_t)cx - (int64_t)VL53LX_p_007 - (int64_t)ax);
