...
void VL53LX_print_nvm_raw_data(uint8_t* pnvm_raw_data, uint32_t trace_flags)
{
    ...
    for (i = 0; i < VL53LX_NVM_SIZE_IN_BYTES; i++)
    {
        ...
        trace_print(VL53LX_TRACE_LEVEL_INFO, "%02X", *pnvm_raw_data++);
    }
...
void VL53LX_print_decoded_nvm_data(
    VL53LX_decoded_nvm_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->nvm__ews__fast_osc_frequency, 12, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->nvm__fmt__fast_osc_frequency, 12, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    {
        sprintf(ppre_text, "%snvm__ews__spad_enables_rtn[%u]", pprefix, i);
    ...
    {
        sprintf(ppre_text, "%snvm__ews__spad_enables_ref__loc1[%u]", pprefix, i);
    ...
    for (i = 0; i < VL53LX_REF_SPAD_BUFFER_SIZE; i++)
    {
        sprintf(ppre_text, "%snvm__ews__spad_enables_ref__loc2[%u]", pprefix, i);
    ...
    for (i = 0; i < VL53LX_REF_SPAD_BUFFER_SIZE; i++)
    {
        sprintf(ppre_text, "%snvm__ews__spad_enables_ref__loc3[%u]", pprefix, i);
    ...
    for (i = 0; i < VL53LX_RTN_SPAD_BUFFER_SIZE; i++)
    {
        sprintf(ppre_text, "%snvm__fmt__spad_enables_rtn[%u]", pprefix, i);
    ...
    for (i = 0; i < VL53LX_REF_SPAD_BUFFER_SIZE; i++)
    {
        sprintf(ppre_text, "%snvm__fmt__spad_enables_ref__loc1[%u]", pprefix, i);
    ...
    for (i = 0; i < VL53LX_REF_SPAD_BUFFER_SIZE; i++)
    {
        sprintf(ppre_text, "%snvm__fmt__spad_enables_ref__loc2[%u]", pprefix, i);
    ...
    for (i = 0; i < VL53LX_REF_SPAD_BUFFER_SIZE; i++)
    {
        sprintf(ppre_text, "%snvm__fmt__spad_enables_ref__loc3[%u]", pprefix, i);
    ...
    VL53LX_print_optical_centre(
        &(pdata->fmt_optical_centre), ppre_text, VL53LX_TRACE_MODULE_NVM_DATA);

    sprintf(ppre_text, "%sfmt_peak_rate_map.", pprefix);

    VL53LX_print_cal_peak_rate_map(
        &(pdata->fmt_peak_rate_map), ppre_text, VL53LX_TRACE_MODULE_NVM_DATA);

    sprintf(ppre_text, "%sfmt_add_offset_data.", pprefix);

    VL53LX_print_additional_offset_cal_data(
        &(pdata->fmt_add_offset_data), ppre_text, VL53LX_TRACE_MODULE_NVM_DATA);
    ...
    VL53LX_print_decoded_nvm_fmt_info(&(pdata->fmt_info), ppre_text, trace_flags);

    sprintf(ppre_text, "%sews_info.", pprefix);

    VL53LX_print_decoded_nvm_ews_info(&(pdata->ews_info), ppre_text, trace_flags);
}

void VL53LX_print_decoded_nvm_fmt_range_data(
    VL53LX_decoded_nvm_fmt_range_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    char fp_text[VL53LX_MAX_STRING_LENGTH];
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->measured_distance_mm, 4, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "measured_distance_mm", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (uint32_t)pdata->measured_distance_stdev_mm, 4, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_decoded_nvm_fmt_info(
    VL53LX_decoded_nvm_fmt_info_t* pdata, char* pprefix, uint32_t trace_flags)
{
    trace_print(
        VL53LX_TRACE_LEVEL_INFO,
        "%s%s = \"%s\"\n",
...
void VL53LX_print_decoded_nvm_ews_info(
    VL53LX_decoded_nvm_ews_info_t* pdata, char* pprefix, uint32_t trace_flags)
{
    trace_print(
        VL53LX_TRACE_LEVEL_INFO,
        "%s%s = \"%s\"\n",
