    ...
    uint8_t* HistMergeNumber)
{
    ...
    VL53LX_init_histogram_bin_data_struct(
        0, pxtalk_shape->xtalk_shape.VL53LX_p_021, &(pxtalk_shape->xtalk_hist_removed));

    VL53LX_copy_xtalk_bin_data_to_histogram_data_struct(
        &(pxtalk_shape->xtalk_shape), &(pxtalk_shape->xtalk_hist_removed));
    ...
    for (r = 0; r <= xtalk_enable; r++)
    {
        ...
        for (t = 0; t < presults->active_results; t++)
        {
            ...
            if (pdata->median_range_mm > ppost_cfg->algo__crosstalk_detect_min_valid_range_mm
                ...
                && delta_mm < ppost_cfg->algo__crosstalk_detect_min_max_tolerance)
            {
    ...
    int16_t* pambient_dmax_mm)
{
    ...
    status = VL53LX_f_001(
        target_reflectance, pdmax_cal, pdmax_cfg, pbins, pdmax_algo, pambient_dmax_mm);
