...
static void vl53lx_diff_histo_stddev(
    VL53LX_LLDriverData_t* pdev,
    VL53LX_histogram_bin_data_t* pdata,
    uint8_t timing,
    uint8_t HighIndex,
    uint8_t prev_pos,
    int32_t* pdiff_histo_stddev)
{
    uint16_t bin = 0;
    ...
    if (pdev->pos_before_next_recom == 0)
    {
        ...
        if (pdev->multi_bins_rec[prev_pos][timing][4] > 0)
            vl53lx_diff_histo_stddev(
                pdev, pdata, timing, HighIndex, prev_pos, &diff_histo_stddev);
        ...
        if (!((recom_been_reset == 1) && (timing == 0)) && (pdev->pos_before_next_recom == 0))
        {
...
VL53LX_Error VL53LX_get_version(VL53LX_DEV Dev, VL53LX_ll_version_t* pdata)
{
...
VL53LX_Error VL53LX_data_init(VL53LX_DEV Dev, uint8_t read_p2p_data)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_init_hist_post_process_config_struct(
            pdev->xtalk_cfg.global_crosstalk_compensation_enable, &(pdev->histpostprocess));
    ...
    VL53LX_init_histogram_bin_data_struct(
        0, VL53LX_HISTOGRAM_BUFFER_SIZE, &(pdev->hist_xtalk));

    VL53LX_init_xtalk_bin_data_struct(
        0, VL53LX_XTALK_HISTO_BINS, &(pdev->xtalk_shapes.xtalk_shape));
    ...
    VL53LX_print_user_zone(
        &(pdev->mm_roi), "data_init():pdev->lldata.mm_roi.", VL53LX_TRACE_MODULE_DATA_INIT);
...
VL53LX_Error VL53LX_read_p2p_data(VL53LX_DEV Dev)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        ...
        status = VL53LX_read_nvm_fmt_range_results_data(
            Dev, VL53LX_NVM__FMT__RANGE_RESULTS__140MM_DARK, &fmt_rrd);
...
VL53LX_Error VL53LX_set_part_to_part_data(VL53LX_DEV Dev, VL53LX_calibration_data_t* pcal_data)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        ...
        memcpy(
            &(pdev->customer), &(pcal_data->customer), sizeof(VL53LX_customer_nvm_managed_t));
        ...
        memcpy(
            &(pdev->gain_cal), &(pcal_data->gain_cal), sizeof(VL53LX_gain_calibration_data_t));
...
VL53LX_Error VL53LX_set_inter_measurement_period_ms(
    VL53LX_DEV Dev, uint32_t inter_measurement_period_ms)
{
...
VL53LX_Error VL53LX_get_inter_measurement_period_ms(
    VL53LX_DEV Dev, uint32_t* pinter_measurement_period_ms)
{
    ...
    uint32_t* prange_config_timeout_us)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        ...
        *pphasecal_config_timeout_us = VL53LX_calc_timeout_us(
            (uint32_t)pdev->gen_cfg.phasecal_config__timeout_macrop, macro_period_us);

        timeout_encoded = (uint16_t)pdev->tim_cfg.mm_config__timeout_macrop_a_hi;
        timeout_encoded =
            (timeout_encoded << 8) + (uint16_t)pdev->tim_cfg.mm_config__timeout_macrop_a_lo;

        *pmm_config_timeout_us =
            VL53LX_calc_decoded_timeout_us(timeout_encoded, macro_period_us);

        timeout_encoded = (uint16_t)pdev->tim_cfg.range_config__timeout_macrop_a_hi;
        timeout_encoded =
            (timeout_encoded << 8) + (uint16_t)pdev->tim_cfg.range_config__timeout_macrop_a_lo;
...
VL53LX_Error VL53LX_get_user_zone(VL53LX_DEV Dev, VL53LX_user_zone_t* puser_zone)
{
...
VL53LX_Error VL53LX_set_zone_config(VL53LX_DEV Dev, VL53LX_zone_config_t* pzone_cfg)
{
    ...
    memcpy(
        &(pdev->zone_cfg.user_zones),
        &(pzone_cfg->user_zones),
    ...
    uint32_t inter_measurement_period_ms)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_set_timeouts_us(
            Dev, phasecal_config_timeout_us, mm_config_timeout_us, range_config_timeout_us);
...
VL53LX_Error VL53LX_enable_xtalk_compensation(VL53LX_DEV Dev)
{
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        pC->crosstalk_range_ignore_threshold_rate_mcps = VL53LX_calc_range_ignore_threshold(
            pC->algo__crosstalk_compensation_plane_offset_kcps,
            pC->algo__crosstalk_compensation_x_plane_gradient_kcps,
            pC->algo__crosstalk_compensation_y_plane_gradient_kcps,
...
void VL53LX_get_xtalk_compensation_enable(
    VL53LX_DEV Dev, uint8_t* pcrosstalk_compensation_enable)
{
...
VL53LX_Error VL53LX_init_and_start_range(
    VL53LX_DEV Dev, uint8_t measurement_mode, VL53LX_DeviceConfigLevel device_config_level)
{
    ...
    psystem->system__mode_start =
        (psystem->system__mode_start & VL53LX_DEVICEMEASUREMENTMODE_STOP_MASK)
        | measurement_mode;

    status =
        VL53LX_set_user_zone(Dev, &(pdev->zone_cfg.user_zones[pdev->ll_state.cfg_zone_id]));

    if (pdev->zone_cfg.active_zones > 0)
    {
        status = VL53LX_set_zone_dss_config(
            Dev, &(pres->zone_dyn_cfgs.VL53LX_p_003[pdev->ll_state.cfg_zone_id]));
    }

    if (((pdev->sys_ctrl.system__mode_start & VL53LX_DEVICESCHEDULERMODE_HISTOGRAM) == 0x00)
    ...
    i2c_buffer_size_bytes =
        (VL53LX_SYSTEM_CONTROL_I2C_INDEX + VL53LX_SYSTEM_CONTROL_I2C_SIZE_BYTES) - i2c_index;
        ...
        && status == VL53LX_ERROR_NONE)
    {
        ...
        status = VL53LX_i2c_encode_static_config(
            pstatic, VL53LX_STATIC_CONFIG_I2C_SIZE_BYTES, &buffer[i2c_buffer_offset_bytes]);
    }
        ...
        && status == VL53LX_ERROR_NONE)
    {
        ...
        status = VL53LX_i2c_encode_general_config(
            pgeneral, VL53LX_GENERAL_CONFIG_I2C_SIZE_BYTES, &buffer[i2c_buffer_offset_bytes]);
    }
        ...
        && status == VL53LX_ERROR_NONE)
    {
        ...
        status = VL53LX_i2c_encode_timing_config(
            ptiming, VL53LX_TIMING_CONFIG_I2C_SIZE_BYTES, &buffer[i2c_buffer_offset_bytes]);
    }
        ...
        && status == VL53LX_ERROR_NONE)
    {
        ...
        if ((psystem->system__mode_start & VL53LX_DEVICEMEASUREMENTMODE_BACKTOBACK)
        ...
        status = VL53LX_i2c_encode_dynamic_config(
            pdynamic, VL53LX_DYNAMIC_CONFIG_I2C_SIZE_BYTES, &buffer[i2c_buffer_offset_bytes]);
    }
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        ...
        status = VL53LX_i2c_encode_system_control(
            psystem, VL53LX_SYSTEM_CONTROL_I2C_SIZE_BYTES, &buffer[i2c_buffer_offset_bytes]);
    }
...
VL53LX_Error VL53LX_stop_range(VL53LX_DEV Dev)
{
    ...
    pdev->sys_ctrl.system__mode_start =
        (pdev->sys_ctrl.system__mode_start & VL53LX_DEVICEMEASUREMENTMODE_STOP_MASK)
...
VL53LX_Error VL53LX_get_measurement_results(
    VL53LX_DEV Dev, VL53LX_DeviceResultsLevel device_results_level)
{
    ...
    case VL53LX_DEVICERESULTSLEVEL_FULL:
        i2c_buffer_size_bytes =
            (VL53LX_DEBUG_RESULTS_I2C_INDEX + VL53LX_DEBUG_RESULTS_I2C_SIZE_BYTES) - i2c_index;
        break;
    case VL53LX_DEVICERESULTSLEVEL_UPTO_CORE:
        i2c_buffer_size_bytes =
            (VL53LX_CORE_RESULTS_I2C_INDEX + VL53LX_CORE_RESULTS_I2C_SIZE_BYTES) - i2c_index;
        break;
    default:
    ...
    VL53LX_range_results_t* prange_results)
{
        ...
        == VL53LX_DEVICESCHEDULERMODE_HISTOGRAM)
    {
        ...
        if ((pdev->tuning_parms.tp_hist_merge == 1) && (histo_merge_nb > 1))
        ...
        status = VL53LX_hist_wrap_dmax(
            &(pdev->histpostprocess), &(pdev->hist_data), &(presults->wrap_dmax_mm));
        ...
        status = VL53LX_hist_phase_consistency_check(
            Dev, &(pZH->VL53LX_p_003[zid]), &(pres->zone_results.VL53LX_p_003[zid]), presults);
        ...
        VL53LX_hist_copy_results_to_sys_and_core(
            &(pdev->hist_data), presults, &(pdev->sys_results), &(pdev->core_results));
    ...
    UPDATE_DYNAMIC_CONFIG:
        if (pzone_cfg->active_zones > 0)
        {
            ...
            for (i = 0; i < VL53LX_MAX_USER_ZONES; i++)
            {
                pzone_cfg->bin_config[i] = ((pdev->ll_state.cfg_internal_stream_count) & 0x01)
...
VL53LX_Error VL53LX_clear_interrupt_and_enable_next_range(
    VL53LX_DEV Dev, uint8_t measurement_mode)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_init_and_start_range(
            Dev, measurement_mode, VL53LX_DEVICECONFIGLEVEL_GENERAL_ONWARDS);
...
VL53LX_Error VL53LX_get_histogram_bin_data(VL53LX_DEV Dev, VL53LX_histogram_bin_data_t* pdata)
{
    ...
    i2c_buffer_offset_bytes =
        VL53LX_PHASECAL_RESULT__REFERENCE_PHASE - VL53LX_HISTOGRAM_BIN_DATA_I2C_INDEX;
    ...
    i2c_buffer_offset_bytes =
        VL53LX_PHASECAL_RESULT__VCSEL_START - VL53LX_HISTOGRAM_BIN_DATA_I2C_INDEX;
    ...
    i2c_buffer_offset_bytes =
        VL53LX_RESULT__HISTOGRAM_BIN_23_0_MSB - VL53LX_HISTOGRAM_BIN_DATA_I2C_INDEX;

    bin_23_0 = buffer[i2c_buffer_offset_bytes] << 2;

    i2c_buffer_offset_bytes =
        VL53LX_RESULT__HISTOGRAM_BIN_23_0_LSB - VL53LX_HISTOGRAM_BIN_DATA_I2C_INDEX;

    bin_23_0 += buffer[i2c_buffer_offset_bytes];

    i2c_buffer_offset_bytes =
        VL53LX_RESULT__HISTOGRAM_BIN_23_0 - VL53LX_HISTOGRAM_BIN_DATA_I2C_INDEX;

    buffer[i2c_buffer_offset_bytes] = bin_23_0;

    i2c_buffer_offset_bytes =
        VL53LX_RESULT__HISTOGRAM_BIN_0_2 - VL53LX_HISTOGRAM_BIN_DATA_I2C_INDEX;
    ...
    if (pdev->ll_state.rd_timing_status == 0)
    {
        ...
        encoded_timeout = (ptim_cfg->range_config__timeout_macrop_a_hi << 8)
    ...
    else
    {
        ...
        encoded_timeout = (ptim_cfg->range_config__timeout_macrop_b_hi << 8)
    ...
    case VL53LX_DEVICEERROR_MULTCLIPFAIL:
        ...
        presults->device_status =
            (pdata->result__range_status & VL53LX_RANGE_STATUS__RANGE_STATUS_MASK);
    ...
    VL53LX_range_results_t* presults)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if ((psys->result__stream_count == 0)
            && ((psys->result__range_status & VL53LX_RANGE_STATUS__RANGE_STATUS_MASK)
...
VL53LX_Error VL53LX_set_zone_dss_config(
    VL53LX_DEV Dev, VL53LX_zone_private_dyn_cfg_t* pzone_dyn_cfg)
{
...
VL53LX_Error VL53LX_get_dmax_mode(VL53LX_DEV Dev, VL53LX_DeviceDmaxMode* pdmax_mode)
{
...
VL53LX_Error VL53LX_get_dmax_calibration_data(
    VL53LX_DEV Dev, VL53LX_DeviceDmaxMode dmax_mode, VL53LX_dmax_calibration_data_t* pdmax_cal)
{
...
VL53LX_Error VL53LX_set_offset_correction_mode(
    VL53LX_DEV Dev, VL53LX_OffsetCorrectionMode offset_cor_mode)
{
...
VL53LX_Error VL53LX_get_offset_correction_mode(
    VL53LX_DEV Dev, VL53LX_OffsetCorrectionMode* poffset_cor_mode)
{
...
VL53LX_Error VL53LX_get_tuning_debug_data(
    VL53LX_DEV Dev, VL53LX_tuning_parameters_t* ptun_data)
{
    ...
    ptun_data->vl53lx_tuningparm_lld_version = pdev->tuning_parms.tp_tuning_parm_lld_version;
    ...
    ptun_data->vl53lx_tuningparm_hist_amb_thresh_sigma_1 = pHP->ambient_thresh_sigma1;

    ptun_data->vl53lx_tuningparm_hist_min_amb_thresh_events = pHP->min_ambient_thresh_events;

    ptun_data->vl53lx_tuningparm_hist_amb_events_scaler = pHP->ambient_thresh_events_scaler;
    ...
    ptun_data->vl53lx_tuningparm_hist_sigma_est_ref_mm = pHP->sigma_estimator__sigma_ref_mm;
    ...
    ptun_data->vl53lx_tuningparm_phasecal_target = pdev->tuning_parms.tp_phasecal_target;

    ptun_data->vl53lx_tuningparm_lite_cal_repeat_rate = pdev->tuning_parms.tp_cal_repeat_rate;

    ptun_data->vl53lx_tuningparm_lite_ranging_gain_factor =
        pdev->gain_cal.standard_ranging_gain_factor;

    ptun_data->vl53lx_tuningparm_lite_min_clip_mm = pdev->tuning_parms.tp_lite_min_clip;
    ...
    ptun_data->vl53lx_tuningparm_lite_sigma_ref_mm = pdev->tuning_parms.tp_lite_sigma_ref_mm;

    ptun_data->vl53lx_tuningparm_lite_rit_mult =
        pdev->xtalk_cfg.crosstalk_range_ignore_threshold_mult;

    ptun_data->vl53lx_tuningparm_lite_seed_config = pdev->tuning_parms.tp_lite_seed_cfg;

    ptun_data->vl53lx_tuningparm_lite_quantifier = pdev->tuning_parms.tp_lite_quantifier;
    ...
    ptun_data->vl53lx_tuningparm_timed_seed_config = pdev->tuning_parms.tp_timed_seed_cfg;
    ...
    ptun_data->vl53lx_tuningparm_refspadchar_vcsel_period = pdev->refspadchar.VL53LX_p_005;
    ...
    ptun_data->vl53lx_tuningparm_xtalk_extract_dss_timeout_us = pXC->mm_config_timeout_us;

    ptun_data->vl53lx_tuningparm_xtalk_extract_bin_timeout_us = pXC->range_config_timeout_us;
    ...
    ptun_data->vl53lx_tuningparm_spadmap_rate_limit_mcps = pdev->ssc_cfg.rate_limit_mcps;
...
VL53LX_Error VL53LX_get_tuning_parm(
    VL53LX_DEV Dev, VL53LX_TuningParms tuning_parm_key, int32_t* ptuning_parm_value)
{
...
VL53LX_Error VL53LX_set_tuning_parm(
    VL53LX_DEV Dev, VL53LX_TuningParms tuning_parm_key, int32_t tuning_parm_value)
{
...
VL53LX_Error VL53LX_get_current_xtalk_settings(
    VL53LX_DEV Dev, VL53LX_xtalk_calibration_results_t* pxtalk)
{
