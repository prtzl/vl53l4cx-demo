...
#define LOG_FUNCTION_END_FMT(status, fmt, ...)                                                \
    _LOG_FUNCTION_END_FMT(VL53LX_TRACE_MODULE_CORE, status, fmt, ##__VA_ARGS__)

#define trace_print(level, ...)                                                               \
    _LOG_TRACE_PRINT(trace_flags, level, VL53LX_TRACE_FUNCTION_NONE, ##__VA_ARGS__)

VL53LX_Error VL53LX_decode_calibration_data_buffer(
    uint16_t buf_size, uint8_t* pbuffer, VL53LX_calibration_data_t* pdata)
{
...
VL53LX_Error VL53LX_get_nvm_debug_data(VL53LX_DEV Dev, VL53LX_decoded_nvm_data_t* pdata)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        VL53LX_print_decoded_nvm_data(
            pdata, "get_nvm_debug_data():pnvm_info.", VL53LX_TRACE_MODULE_NVM_DATA);
#endif
...
VL53LX_Error VL53LX_get_histogram_debug_data(
    VL53LX_DEV Dev, VL53LX_histogram_bin_data_t* pdata)
{
...
VL53LX_Error VL53LX_get_xtalk_debug_data(VL53LX_DEV Dev, VL53LX_xtalk_debug_data_t* pdata)
{
    ...
    memcpy(&(pdata->customer), &(pdev->customer), sizeof(VL53LX_customer_nvm_managed_t));

    memcpy(&(pdata->xtalk_cfg), &(pdev->xtalk_cfg), sizeof(VL53LX_xtalk_config_t));

    memcpy(&(pdata->hist_data), &(pdev->hist_data), sizeof(VL53LX_histogram_bin_data_t));

    memcpy(
        &(pdata->xtalk_shapes), &(pdev->xtalk_shapes), sizeof(VL53LX_xtalk_histogram_data_t));

    memcpy(
        &(pdata->xtalk_results), &(pdev->xtalk_results), sizeof(VL53LX_xtalk_range_results_t));
...
VL53LX_Error VL53LX_get_offset_debug_data(VL53LX_DEV Dev, VL53LX_offset_debug_data_t* pdata)
{
    ...
    memcpy(
        &(pdata->fmt_dmax_cal), &(pdev->fmt_dmax_cal), sizeof(VL53LX_dmax_calibration_data_t));
...
void VL53LX_signed_fixed_point_sprintf(
    int32_t signed_fp_value, uint8_t frac_bits, uint16_t buf_size, char* pbuffer)
{
...
void VL53LX_print_static_nvm_managed(
    VL53LX_static_nvm_managed_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_customer_nvm_managed(
    VL53LX_customer_nvm_managed_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf((int32_t)tmpi16, 11, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_nvm_copy_data(
    VL53LX_nvm_copy_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_histogram_bin_data(
    VL53LX_histogram_bin_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_019", pdata->VL53LX_p_019);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_020", pdata->VL53LX_p_020);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_021", pdata->VL53LX_p_021);
    ...
    for (i = 0; i < VL53LX_MAX_BIN_SEQUENCE_LENGTH; i++)
    {
        ...
        trace_print(VL53LX_TRACE_LEVEL_INFO, "%s = %u\n", ppre_text, pdata->bin_seq[i]);
    }

    for (i = 0; i < VL53LX_MAX_BIN_SEQUENCE_LENGTH; i++)
    {
        sprintf(ppre_text, "%sbin_rep[%u]", pprefix, i);
        trace_print(VL53LX_TRACE_LEVEL_INFO, "%s = %u\n", ppre_text, pdata->bin_rep[i]);
    }

    for (i = 0; i < pdata->VL53LX_p_021; i++)
    {
        sprintf(ppre_text, "%sbin_data[%u]", pprefix, i);
        trace_print(VL53LX_TRACE_LEVEL_INFO, "%s = %d\n", ppre_text, pdata->bin_data[i]);
    }
    ...
    VL53LX_signed_fixed_point_sprintf(
        (uint32_t)pdata->vcsel_width, 4, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "vcsel_width", fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_005", pdata->VL53LX_p_005);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_015, 12, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "VL53LX_p_015", fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->zero_distance_phase, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "zero_distance_phase", fp_text);
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %d\n", pprefix, "VL53LX_p_028", pdata->VL53LX_p_028);
...
void VL53LX_print_xtalk_histogram_shape_data(
    VL53LX_xtalk_histogram_shape_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    for (i = 0; i < pdata->VL53LX_p_021; i++)
    {
        ...
        VL53LX_signed_fixed_point_sprintf(
            (int32_t)pdata->bin_data[i], 10, VL53LX_MAX_STRING_LENGTH, fp_text);

        trace_print(VL53LX_TRACE_LEVEL_INFO, "%s = %s\n", ppre_text, fp_text);
    }
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->vcsel_width, 4, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_xtalk_histogram_data(
    VL53LX_xtalk_histogram_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_print_xtalk_histogram_shape_data(&(pdata->xtalk_shape), ppre_text, trace_flags);

    sprintf(ppre_text, "%sxtalk_hist_removed.", pprefix);
    VL53LX_print_histogram_bin_data(&(pdata->xtalk_hist_removed), ppre_text, trace_flags);
}

void VL53LX_print_range_data(VL53LX_range_data_t* pdata, char* pprefix, uint32_t trace_flags)
{

    char fp_text[VL53LX_MAX_STRING_LENGTH];

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "range_id", pdata->range_id);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "time_stamp", pdata->time_stamp);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_012", pdata->VL53LX_p_012);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_019", pdata->VL53LX_p_019);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_023", pdata->VL53LX_p_023);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_024", pdata->VL53LX_p_024);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_013", pdata->VL53LX_p_013);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_025", pdata->VL53LX_p_025);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->width, 4, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "width", fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "VL53LX_p_029", pdata->VL53LX_p_029);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->fast_osc_frequency, 12, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_004, 8, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %d\n", pprefix, "VL53LX_p_016", pdata->VL53LX_p_016);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %d\n", pprefix, "VL53LX_p_017", pdata->VL53LX_p_017);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %d\n", pprefix, "VL53LX_p_010", pdata->VL53LX_p_010);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->peak_signal_count_rate_mcps, 7, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->avg_signal_count_rate_mcps, 7, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->ambient_count_rate_mcps, 7, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "ambient_count_rate_mcps", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->total_rate_per_spad_mcps, 13, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "total_rate_per_spad_mcps", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_009, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "VL53LX_p_009", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_002, 2, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "VL53LX_p_002", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_026, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "VL53LX_p_026", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_011, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "VL53LX_p_011", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->VL53LX_p_027, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "VL53LX_p_027", fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %d\n", pprefix, "min_range_mm", pdata->min_range_mm);
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %d\n", pprefix, "max_range_mm", pdata->max_range_mm);
...
void VL53LX_print_range_results(
    VL53LX_range_results_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    for (i = 0; i < VL53LX_MAX_AMBIENT_DMAX_VALUES; i++)
    {
        ...
        trace_print(VL53LX_TRACE_LEVEL_INFO, "%s = %u\n", ppre_text, pdata->VL53LX_p_022[i]);
    }
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "wrap_dmax_mm", pdata->wrap_dmax_mm);
    ...
    for (i = 0; i < pdata->active_results; i++)
    {
        ...
        VL53LX_print_range_data(&pdata->VL53LX_p_003[i], ppre_text, trace_flags);
    }

    sprintf(ppre_text, "%sxmonitor.", pprefix);
    VL53LX_print_range_data(&pdata->xmonitor, ppre_text, trace_flags);
}

void VL53LX_print_offset_range_results(
    VL53LX_offset_range_results_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->cal_reflectance_pc, 2, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "cal_reflectance_pc", fp_text);
    ...
    {
        sprintf(ppre_text, "%sdata[%u].", pprefix, i);
        VL53LX_print_offset_range_data(&(pdata->VL53LX_p_003[i]), ppre_text, trace_flags);
    }
}

void VL53LX_print_offset_range_data(
    VL53LX_offset_range_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->effective_spads, 8, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "effective_spads", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->peak_rate_mcps, 7, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_cal_peak_rate_map(
    VL53LX_cal_peak_rate_map_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->cal_distance_mm, 2, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    for (y = 0; y < pdata->height; y++)
    {
        for (x = 0; x < pdata->width; x++)
        {
            ...
            VL53LX_signed_fixed_point_sprintf(
                (int32_t)pdata->peak_rate_mcps[i], 7, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_additional_data(
    VL53LX_additional_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "zone_preset", pdata->zone_preset);
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "dmax_mode", pdata->dmax_mode);
    ...
    VL53LX_print_histogram_bin_data(&pdata->VL53LX_p_006, ppre_text, trace_flags);
}

void VL53LX_print_additional_offset_cal_data(
    VL53LX_additional_offset_cal_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_gain_calibration_data(
    VL53LX_gain_calibration_data_t* pdata, char* pprefix, uint32_t trace_flags)
{

    char fp_text[VL53LX_MAX_STRING_LENGTH];

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->standard_ranging_gain_factor, 11, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->histogram_ranging_gain_factor, 11, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_zone_calibration_data(
    VL53LX_zone_calibration_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->median_range_mm, 2, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "median_range_mm", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->range_mm_offset, 2, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "range_mm_offset", fp_text);
}

void VL53LX_print_zone_calibration_results(
    VL53LX_zone_calibration_results_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "max_zones", pdata->max_zones);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %u\n", pprefix, "active_zones", pdata->active_zones);

    for (i = 0; i < pdata->active_zones; i++)
    {
        sprintf(ppre_text, "%sdata[%u].", pprefix, i);
        VL53LX_print_zone_calibration_data(&(pdata->VL53LX_p_003[i]), ppre_text, trace_flags);
    }
}

void VL53LX_print_xtalk_range_results(
    VL53LX_xtalk_range_results_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    {
        sprintf(ppre_text, "%sdata[%u].", pprefix, i);
        VL53LX_print_xtalk_range_data(&(pdata->VL53LX_p_003[i]), ppre_text, trace_flags);
    }

    sprintf(ppre_text, "%scentral_histogram_sum.", pprefix);
    VL53LX_print_histogram_bin_data(&pdata->central_histogram_sum, ppre_text, trace_flags);

    sprintf(ppre_text, "%scentral_histogram_avg.", pprefix);
    VL53LX_print_histogram_bin_data(&pdata->central_histogram_avg, ppre_text, trace_flags);
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        VL53LX_print_histogram_bin_data(pbin_data, ppre_text, trace_flags);
        pbin_data++;
    }
...
void VL53LX_print_xtalk_range_data(
    VL53LX_xtalk_range_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->rate_per_spad_kcps_sum, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "rate_per_spad_kcps_sum", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->rate_per_spad_kcps_avg, 11, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "rate_per_spad_kcps_avg", fp_text);
}

void VL53LX_print_xtalk_calibration_results(
    VL53LX_xtalk_calibration_results_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_xtalk_config(
    VL53LX_xtalk_config_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->lite_mode_crosstalk_margin_kcps, 9, VL53LX_MAX_STRING_LENGTH, fp_text);
...
void VL53LX_print_xtalk_extract_config(
    VL53LX_xtalkextract_config_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_zone_cal_config(
    VL53LX_zonecal_config_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_offset_cal_config(
    VL53LX_offsetcal_config_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_dmax_calibration_data(
    VL53LX_dmax_calibration_data_t* pdata, char* pprefix, uint32_t trace_flags)
{

    char fp_text[VL53LX_MAX_STRING_LENGTH];

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->ref__actual_effective_spads, 8, VL53LX_MAX_STRING_LENGTH, fp_text);
    ...
    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->ref__distance_mm, 4, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "ref__distance_mm", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->ref_reflectance_pc, 2, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "ref_reflectance_pc", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->coverglass_transmission, 8, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(
        VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "coverglass_transmission", fp_text);
}

void VL53LX_print_calibration_data(
    VL53LX_calibration_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_print_customer_nvm_managed(&(pdata->customer), ppre_text, trace_flags);

    sprintf(ppre_text, "%sfmt_dmax_cal.", pprefix);
    VL53LX_print_dmax_calibration_data(&(pdata->fmt_dmax_cal), ppre_text, trace_flags);

    sprintf(ppre_text, "%scust_dmax_cal.", pprefix);
    VL53LX_print_dmax_calibration_data(&(pdata->cust_dmax_cal), ppre_text, trace_flags);

    sprintf(ppre_text, "%sadd_off_cal_data.", pprefix);
    VL53LX_print_additional_offset_cal_data(
        &(pdata->add_off_cal_data), ppre_text, trace_flags);

    sprintf(ppre_text, "%soptical_centre.", pprefix);
    VL53LX_print_optical_centre(&(pdata->optical_centre), ppre_text, trace_flags);

    sprintf(ppre_text, "%sxtalkhisto.", pprefix);
    VL53LX_print_xtalk_histogram_data(&(pdata->xtalkhisto), ppre_text, trace_flags);

    sprintf(ppre_text, "%sgain_cal.", pprefix);
    VL53LX_print_gain_calibration_data(&(pdata->gain_cal), ppre_text, trace_flags);

    sprintf(ppre_text, "%scal_peak_rate_map.", pprefix);
    VL53LX_print_cal_peak_rate_map(&(pdata->cal_peak_rate_map), ppre_text, trace_flags);
}

void VL53LX_print_xtalk_debug_data(
    VL53LX_xtalk_debug_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_print_xtalk_config(&(pdata->xtalk_cfg), ppre_text, trace_flags);

    sprintf(ppre_text, "%sxtalk_extract_cfg.", pprefix);
    VL53LX_print_xtalk_extract_config(&(pdata->xtalk_extract_cfg), ppre_text, trace_flags);

    sprintf(ppre_text, "%shist_data.", pprefix);
    VL53LX_print_histogram_bin_data(&(pdata->hist_data), ppre_text, trace_flags);

    sprintf(ppre_text, "%sxtalk_shapes.", pprefix);
    VL53LX_print_xtalk_histogram_data(&(pdata->xtalk_shapes), ppre_text, trace_flags);

    sprintf(ppre_text, "%sgain_cal.", pprefix);
    VL53LX_print_xtalk_range_results(&(pdata->xtalk_results), ppre_text, trace_flags);
}

void VL53LX_print_offset_debug_data(
    VL53LX_offset_debug_data_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    VL53LX_print_offset_range_results(&(pdata->offset_results), ppre_text, trace_flags);
}

void VL53LX_print_zone_config(VL53LX_zone_config_t* pdata, char* pprefix, uint32_t trace_flags)
{
    ...
    for (i = 0; i < pdata->active_zones; i++)
    {
        ...
        VL53LX_print_user_zone(&pdata->user_zones[i], ppre_text, trace_flags);
    }
}

void VL53LX_print_optical_centre(
    VL53LX_optical_centre_t* pdata, char* pprefix, uint32_t trace_flags)
{

    char fp_text[VL53LX_MAX_STRING_LENGTH];

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->x_centre, 4, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "x_centre", fp_text);

    VL53LX_signed_fixed_point_sprintf(
        (int32_t)pdata->y_centre, 4, VL53LX_MAX_STRING_LENGTH, fp_text);

    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%s = %s\n", pprefix, "y_centre", fp_text);
}

void VL53LX_print_user_zone(VL53LX_user_zone_t* pdata, char* pprefix, uint32_t trace_flags)
{
...
void VL53LX_print_spad_rate_data(
    VL53LX_spad_rate_data_t* pspad_rates, char* pprefix, uint32_t trace_flags)
{
    ...
    for (spad_no = 0; spad_no < pspad_rates->no_of_values; spad_no++)
    {
        ...
        VL53LX_decode_row_col((uint8_t)spad_no, &row, &col);
    ...
    VL53LX_spad_rate_data_t* pspad_rates, char* pprefix, uint32_t trace_flags)
{
    ...
    trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%4s", pprefix, " ");

    for (col = 0; col < VL53LX_SPAD_ARRAY_WIDTH; col++)
        trace_print(VL53LX_TRACE_LEVEL_INFO, ",%8u", col);
    ...
    for (row = 0; row < VL53LX_SPAD_ARRAY_HEIGHT; row++)
    {
        ...
        trace_print(VL53LX_TRACE_LEVEL_INFO, "%s%4u", pprefix, row);

        for (col = 0; col < VL53LX_SPAD_ARRAY_HEIGHT; col++)
        {

            VL53LX_encode_row_col(row, col, &spad_no);
            ...
            trace_print(VL53LX_TRACE_LEVEL_INFO, ",%8s", fp_text);
        }
