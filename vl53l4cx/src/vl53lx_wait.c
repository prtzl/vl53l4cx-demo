...
VL53LX_Error VL53LX_wait_for_boot_completion(VL53LX_DEV Dev)
{
    ...
    else
    {
        ...
        {
            status = VL53LX_is_boot_complete(Dev, &fw_ready);
...
VL53LX_Error VL53LX_wait_for_test_completion(VL53LX_DEV Dev)
{
    ...
    if (pdev->wait_method == VL53LX_WAIT_METHOD_BLOCKING)
    {
        ...
        status =
            VL53LX_poll_for_range_completion(Dev, VL53LX_TEST_COMPLETION_POLLING_TIMEOUT_MS);
    }
    else
    {
...
VL53LX_Error VL53LX_is_boot_complete(VL53LX_DEV Dev, uint8_t* pready)
{
    ...
    status = VL53LX_RdByte(Dev, VL53LX_FIRMWARE__SYSTEM_STATUS, &firmware__system_status);
    ...
    {
        *pready = 0x00;
        VL53LX_init_ll_driver_state(Dev, VL53LX_DEVICESTATE_FW_COLDBOOT);
    }
...
VL53LX_Error VL53LX_is_firmware_ready(VL53LX_DEV Dev, uint8_t* pready)
{
...
VL53LX_Error VL53LX_is_new_data_ready(VL53LX_DEV Dev, uint8_t* pready)
{
    ...
    gpio__mux_active_high_hv =
        pdev->stat_cfg.gpio_hv_mux__ctrl & VL53LX_DEVICEINTERRUPTLEVEL_ACTIVE_MASK;
...
VL53LX_Error VL53LX_poll_for_firmware_ready(VL53LX_DEV Dev, uint32_t timeout_ms)
{
    ...
    while ((status == VL53LX_ERROR_NONE) && (pdev->fw_ready_poll_duration_ms < timeout_ms)
           && (fw_ready == 0))
    {

        status = VL53LX_is_firmware_ready(Dev, &fw_ready);

        if (status == VL53LX_ERROR_NONE && fw_ready == 0 && poll_delay_ms > 0)
        {
