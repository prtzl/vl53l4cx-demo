...
VL53LX_Error VL53LX_run_ref_spad_char(VL53LX_DEV Dev, VL53LX_Error* pcal_status)
{
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_ReadMulti(
            Dev, VL53LX_REF_SPAD_CHAR_RESULT__NUM_ACTUAL_REF_SPADS, comms_buffer, 2);
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_WriteMulti(
            Dev, VL53LX_REF_SPAD_MAN__NUM_REQUESTED_REF_SPADS, comms_buffer, 2);
    ...
    if (status == VL53LX_ERROR_NONE)
        status =
            VL53LX_WriteMulti(Dev, VL53LX_GLOBAL_CONFIG__SPAD_ENABLES_REF_0, comms_buffer, 6);
    ...
    *pcal_status = status;
    ...
    IGNORE_STATUS(
        IGNORE_REF_SPAD_CHAR_RATE_TOO_LOW, VL53LX_WARNING_REF_SPAD_CHAR_RATE_TOO_LOW, status);
    ...
    VL53LX_histogram_bin_data_t* pavg_histo)
{
    ...
    VL53LX_range_results_t* prs = (VL53LX_range_results_t*)pdev->wArea1;
    ...
    if (status == VL53LX_ERROR_NONE)
        status = VL53LX_init_and_start_range(
            Dev, measurement_mode, VL53LX_DEVICECONFIGLEVEL_CUSTOMER_ONWARDS);
    ...
    for (i = 0; i <= (final_zone * num_of_samples); i++)
    {
        ...
        if (status == VL53LX_ERROR_NONE)
            status = VL53LX_get_device_results(Dev, VL53LX_DEVICERESULTSLEVEL_FULL, prs);

        if (status == VL53LX_ERROR_NONE
            && pdev->ll_state.rd_device_state != VL53LX_DEVICESTATE_RANGING_WAIT_GPH_SYNC)
        {
            ...
            if (prs->active_results > 1)
            {
                ...
                {
                    if (prs->VL53LX_p_003[j].median_range_mm < prange_data->median_range_mm)
                        prange_data = &(prs->VL53LX_p_003[j]);
                }
            }
            ...
            if ((prs->active_results > 0)
                && (prange_data->median_range_mm < xtalk_filter_thresh_max_mm)
                && (prange_data->median_range_mm > xtalk_filter_thresh_min_mm)
                && (prange_data->VL53LX_p_009 < (uint32_t)(xtalk_max_valid_rate_kcps * 16)))
                valid_result = 1;
            else
                valid_result = 0;
            ...
            if (valid_result == 1)
            {
                ...
                pxtalk_range_data->rate_per_spad_kcps_sum += prange_data->VL53LX_p_009;

                pxtalk_range_data->signal_total_events_sum += prange_data->VL53LX_p_010;

                pxtalk_range_data->sigma_mm_sum += (uint32_t)prange_data->VL53LX_p_002;

                pxtalk_range_data->median_phase_sum += (uint32_t)prange_data->VL53LX_p_011;
            }

            if ((valid_result == 1) && (zone_id >= 4))
            {
                status = VL53LX_sum_histogram_data(&(pdev->hist_data), psum_histo);

                if (prange_data->VL53LX_p_012 < pXR->central_histogram__window_start)
                    pXR->central_histogram__window_start = prange_data->VL53LX_p_012;

                if (prange_data->VL53LX_p_013 > pXR->central_histogram__window_end)
        ...
        if (status == VL53LX_ERROR_NONE)
        {
            VL53LX_print_range_results(
                &(pres->range_results), "pres->range_results.", VL53LX_TRACE_MODULE_CORE);
        }
#endif
    ...
    for (i = 0; i < (pdev->zone_cfg.active_zones + 1); i++)
    {
        ...
        if (pxtalk_range_data->no_of_samples > 0)
        {
            ...
            pxtalk_range_data->sigma_mm_avg =
                pxtalk_range_data->sigma_mm_sum / (uint32_t)pxtalk_range_data->no_of_samples;

            pxtalk_range_data->median_phase_avg = pxtalk_range_data->median_phase_sum
                                                / (uint32_t)pxtalk_range_data->no_of_samples;
        }
        else
        {
    ...
    memcpy(pavg_histo, &(pdev->hist_data), sizeof(VL53LX_histogram_bin_data_t));
    ...
    if (status == VL53LX_ERROR_NONE)
    {
        ...
        status = VL53LX_avg_histogram_data(
            pxtalk_range_data->no_of_samples, psum_histo, pavg_histo);
    }
    ...
    uint16_t* pzero_distance_phase)
{
    ...
    for (m = 0; m < phasecal_num_of_samples; m++)
    {
        ...
        for (i = 0; i <= 1; i++)
        {
            ...
            if (status == VL53LX_ERROR_NONE)
                status = VL53LX_get_device_results(
                    Dev, VL53LX_DEVICERESULTSLEVEL_FULL, prange_results);
        ...
        if (status == VL53LX_ERROR_NONE)
        {
            ...
            period = 2048 * (uint32_t)VL53LX_decode_vcsel_period(pdev->hist_data.VL53LX_p_005);

            VL53LX_p_014 = period;
            VL53LX_p_014 += (uint32_t)(pdev->hist_data.phasecal_result__reference_phase);
            VL53LX_p_014 += (2048 * (uint32_t)phasecal_result__vcsel_start);
            VL53LX_p_014 -= (2048 * (uint32_t)pdev->hist_data.cal_config__vcsel_start);
    ...
    if (status == VL53LX_ERROR_NONE && samples > 0)
    {
        ...
        *pphasecal_result__reference_phase = (uint16_t)phasecal_result__reference_phase;
    ...
    VL53LX_hist_xtalk_extract_data_t* pxtalk_data)
{
    ...
    status = VL53LX_hist_xtalk_extract_calc_window(
        target_distance_mm, target_width_oversize, phist_bins, pxtalk_data);
...
VL53LX_Error VL53LX_run_hist_xtalk_extraction(
    VL53LX_DEV Dev, int16_t cal_distance_mm, VL53LX_Error* pcal_status)
{
...
#define OVERSIZE 4
    ...
    for (k = 0; k < nbloops; k++)
    {
        ...
        for (i = 0; i <= pX->num_of_samples; i++)
        {
            ...
            wait_for_accumulation =
                ((k != 0) && (MergeEnabled) && (status == VL53LX_ERROR_NONE)
            ...
            else
            {
                if ((status == VL53LX_ERROR_NONE) && (!Very1stRange))
                {
                    status = VL53LX_hist_xtalk_extract_update(
                        cal_distance_mm, OVERSIZE, &(pdev->hist_data), &(pdev->xtalk_extract));
                }
            }
    ...
    *pcal_status = pdev->xtalk_results.cal_status;
    ...
    VL53LX_print_xtalk_histogram_data(
        &(pdev->xtalk_shapes), "pdev->lldata.xtalk_shapes.", VL53LX_TRACE_MODULE_XTALK_DATA);
